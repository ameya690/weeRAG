...
class HeuristicJudge:
    ...
    def score(self, question: str, answer: str, references: list[str]) -> float:
        ...
        av = self.embed(answer)
        refs = np.vstack([self.embed(r) for r in references]).astype(np.float32)
        sims = refs @ av.reshape(-1, 1)
        sims = (sims + 1.0) * 0.5
...
class Judge:
    def __init__(
        ...
        self.llm_fn = llm_fn
        self.rubric = rubric or "Return a score [0,1]."
    ...
    def judge(
        ...
        if self.llm_fn is not None:
            ...
            for _ in range(n):
                scores.append(max(0.0, min(1.0, float(self.llm_fn(prompt)))))
        else:
            for _ in range(n):
                scores.append(self.heuristic.score(question, answer, references))
